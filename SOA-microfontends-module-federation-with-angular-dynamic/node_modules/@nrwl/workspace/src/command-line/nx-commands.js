...
const detect_package_manager_1 = require("@nrwl/workspace/src/utils/detect-package-manager");
...
- Run migrations (e.g., nx migrate --run-migrations=migrations.json)
    ...
    if (process.env.NX_MIGRATE_USE_LOCAL === undefined) {
        ...
        const pmc = detect_package_manager_1.getPackageManagerExecuteCommand();
        child_process_1.execSync(`${pmc} tao migrate ${process.argv.slice(3).join(' ')}`, {
            stdio: ['inherit', 'inherit', 'inherit'],
        });
    }
})
...
function taoPath() {
    ...
    fs_extra_1.writeFileSync(path.join(tmpDir, 'package.json'), JSON.stringify({
        dependencies: {
            '@nrwl/tao': 'latest',
